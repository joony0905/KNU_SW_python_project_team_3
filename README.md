...
##   2. 주요 기능
   ...
   * **2.2. 모델 선택:** 사용자는 분류를 위해 **빠른 판단(나이브 베이즈)** 및 **정밀 판단(랜덤 포레스트)** 중에서 선택할 수 있습니다.<br><br>
   ...
   * **2.4. 나이브 베이즈 분류:** 나이브 베이즈 모델이 스팸 분류에 사용됩니다. 구현에는 다음이 포함됩니다.
        ...
        * 2.4.2. 로그 확률 계산 및 정규화  **···** ✅25_05_17 완료 <br>
          (문장이 길어질 경우 많은 단어의 출현으로 인해 편향된 확률을 나타냄을 확인하여, <br> [로그 확률 / 문장 길이]로 정규화 하는 작업을 진행하였습니다.) <br><br>

   * **2.5. 랜덤 포레스트 분류:** 랜덤 포레스트 모델이 스팸 분류에 사용됩니다. <br> **···** ✅ 25_05_28 기준 랜덤 포레스트 구현 마무리(Beta) <br><br>
   나이브 베이즈 모델은 단어의 등장 여부만을 기반으로 예측하기 때문에, <br> 정상 광고 문자와 스팸 광고 문자의 미묘한 차이를 구분하는 데 한계가 있었습니다. <br> 이에 따라, 서로 다른 관점에서 접근하는 랜덤 포레스트 모델을 추가하여 <br> 두 모델      의 성능을 비교하고자 하였습니다.
        * 2.5.1. 텍스트 데이터의 Count 벡터화 **···** ✅ 25_05_09 완료 <br>
        * 2.5.2. 정상 문자를 카테고리 별로 분류 후 One_Hot Encoding, hstack 결합 **···** ✅ 25_05_27 기준 완료 <br>
        * 2.5.3. 문장 길이 피처 추가 및 softmax로 확률 평탄화 ✅ 25_05_27 기준 완료 <br>
        * 2.5.4. 하이퍼파라미터 튜닝을 통해 과적합 현상 보완 ✅ 25_05_27 기준 완료 <br>
   ...
   * 🤦‍♀️ 시행착오들..     
     <details><summary>[>] </summary>
      ...
      - 추후에 팀원들중 만약 대학원 진학을 하신다면.. 문맥을 더 잘 파악할 수 있는 딥러닝 기반 모델을 도입해보면 어떨까 싶습니다.
      ...
      **카테고리 적용후**
      ...
      #### 확률 보정
      - **Naive Bayes**:
        - 로그 확률을 적용하여 확률 과잉 확신 현상을 보정했습니다.
      - **Random Forest**:
      ...
      - +) 파라미터 최적화를 해주는 `GridSearchCV`가 있다는 것을 뒤늦게 깨달았습니다. 추후에 적용해 보고싶습니다..
...
3.  **데이터 및 모델 확인:**
    ...
    * 5.3.2. 학습된 모델 (`spam_classifier.pkl`) 및 벡터화기 (`vectorizer.pkl`)가 `model/` 디렉토리에 있는지 확인합니다. <br> 없는 경우 `model/train_model.py`를 Run해 각 모델을 학습하여 생성합니다.
...
**<details><summary> 📁 6.1. app </summary>**
...
**<details><summary> 📁 6.2. model</summary>**
...
**<details><summary> 📁 6.3. utils</summary>**
...
* 7.1. GridSearchCV로 Random Forest 하이퍼파라미터 튜닝
* 7.2. feedback을 통해 받은 클라이언트측 데이터를 통해 재학습 (⭐)
* 7.3. 문맥 인지를 위한 딥러닝 모델 추가
