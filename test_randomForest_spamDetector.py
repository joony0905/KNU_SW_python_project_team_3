...
stop_words = {
    ...
    '힘입어', '의', '가', '이', '은', '는', '을', '를', '에', '와', '과', '한', '하다', '있다', '없다', 
...
normal_files = [
    ("ad_messages.csv", "ad"),
    ("bank_messages.csv", "bank"),
    ("parcel_messages.csv", "parcel"),
    ("telecom_messages.csv", "telcom"),
    ("university_messages.csv", "university")
]
normal_dfs = [load_normal(path, cat) for path, cat in normal_files]
normal_df = pd.concat(normal_dfs, ignore_index=True)

# 3. 스팸 메시지 로딩
spam_df = pd.read_csv('init_phishing.csv', usecols=["Spam message"], encoding='cp949')
spam_df = spam_df.rename(columns={"Spam message": "message"})
spam_df['label'] = 'spam'
spam_df['category'] = 'spam'
spam_df = spam_df.sample(n=32000, random_state=42)

# 4. 병합 및 전처리
combined_df = pd.concat([normal_df, spam_df], ignore_index=True)
combined_df['message'] = combined_df['message'].astype(str).apply(preprocess_text)
combined_df['label'] = combined_df['label'].map({'ham': 0, 'spam': 1})

# 5. One-hot encoding for category
category_onehot = pd.get_dummies(combined_df['category'])

# 6. 훈련/테스트 분할 (벡터화 이전에!)
X_text = combined_df['message']
y = combined_df['label']
X_cat = category_onehot.values
...
# 8. TF-IDF + Category One-hot 합치기
X_train = hstack([X_tfidf_train, csr_matrix(X_cat_train)])
X_test = hstack([X_tfidf_test, csr_matrix(X_cat_test)])
...
# ✅ 모델 저장
joblib.dump(model, 'rf_spam_classifier.pkl')  # 스팸 분류기
joblib.dump(vectorizer, 'rf_tfidf_vectorizer.pkl')  # TF-IDF 벡터라이저
joblib.dump(category_onehot.columns.tolist(), 'rf_category_columns.pkl')  # One-hot 컬럼 이름
...
# 11. 예측 함수
def predict_message(message, category='ad'):
    processed = preprocess_text(message)
    vec = vectorizer.transform([processed])

    onehot = pd.DataFrame([[0] * len(category_onehot.columns)], columns=category_onehot.columns)
    if category in onehot.columns:
        onehot[category] = 1

    category_sparse = csr_matrix(onehot.values)
    final_vec = hstack([vec, category_sparse])

    pred = model.predict(final_vec)[0]
    prob = model.predict_proba(final_vec)[0]
    print(f"입력 메시지: {message}")
    print(f"예측: {'SPAM' if pred else 'HAM'}")
    print(f"확률: spam {prob[1]*100:.2f}%, ham {prob[0]*100:.2f}%")
    return pred

predict_message("모바일 청첩장 저희 결혼합니다. 일시:4월 27일 (목) http://wedding.com/fofs")
